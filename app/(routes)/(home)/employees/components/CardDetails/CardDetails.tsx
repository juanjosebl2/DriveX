...
import { suggestionsData } from "../Suggestions/Suggestions.data";
import { CardDetailsProps } from "./CardDetails.types";

export function CardDetails( props : CardDetailsProps) {

  const { selectedEmployed } = props;

  const employee = suggestionsData.find(emp => emp.id === selectedEmployed);
  ...
  return (
    <div>
      <div className="mb-4 font-sans font-bold">Detalles</div>
      <div className="flex flex-col p-4 shadow-lg sm:flex-row rounded-xl hover:shadow-lg">
        <Image
          src={employee.photo}
          alt={employee.name}
          width={400}
          height={600}
          className="rounded-lg lg:w-1/5"
        />
        <div className="flex flex-col justify-between p-10 lg:text-2xl">
          <div className="mb-5 font-bold lg:mb-2 ">{employee.name}</div>
          ...
          <div>
            <div className="flex items-center justify-center w-full h-auto gap-4 py-8 bg-white lg:flex-wrap">
              <button className="transition-all duration-500 group hover:-translate-y-2">
                <svg
          ...
          <Button className="rounded-xl">Pedir cita</Button>
        </div>
      </div>
    </div>
  );
